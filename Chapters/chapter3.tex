...
The annotation may contain parameters, these allow to add configuration when the declaration is used.
Supported types are:
\begin{compactitem}
...
ExtendJ is an extensible compiler, its goals are to facilitate the development of Java compiler tools.
...
The \emph{Analysis} phase is occurs after parsing, when types are checked.
...
this is due to the amount of detail required to take into account.
...
some approaches aim to bridge modern languages and behavioral types,
...
The transpilation process takes Scribble local protocols as input, it outputs Mungo typestate specifications and Java skeleton implementation code.
...
such work was limited since it only supported binary session types.
...
Vault introduces two new concepts, \emph{adoption} and \emph{focus}, these two serve to relax constrains imposed by a linear type system.
...
this way, all aliases lifetimes are tied to the lifetime of the adopter.
...
It also introduces the \keyword{shared} keyword, which makes an object behave like it normally would in Java,

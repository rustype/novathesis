...
\paragraph{State declaration} uses the \keyword{struct} keyword.
...
From the code, we can establish edges in the automata from the \keyword{self} parameter and the return function.
...
The final state is similar in that instead of the \keyword{self} being absent, the return type is missing.
